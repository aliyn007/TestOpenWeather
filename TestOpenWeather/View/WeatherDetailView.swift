...
class WeatherDetailView: FancyView {
    ...
    func updateHours(hourly: [Hourly]) {
        ...
        for i in 0...4 {
            ...
            let weatherTemperature = "\(Int(hour.tempCelsiusHourly))°C"
            
            topLabels[i]?.text = hourString
            images[i]?.image = UIImage(named: weatherIconName)
            bottomLabels[i]?.text = weatherTemperature
        }
    ...
    func updateDays(daily: [Daily]) {
        ...
        for i in 0...4 {
            ...
            let weatherTemperature = day.temp.tempCelsiusDay
            
            topLabels[i]?.text = dayString
            images[i]?.image = UIImage(named: weatherIconName)
            bottomLabels[i]?.text = "\(Int(weatherTemperature))°С"
        }
    }
}
